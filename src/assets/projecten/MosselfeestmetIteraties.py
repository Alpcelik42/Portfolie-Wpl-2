""" Alperen ozcelik """
#Inlezen van waardes 
naam = str(input('Geef de naam van bediende in: '))
kassaOp = int(input('Geef beginsituatie van het kassa: '))
print('Hallo',naam,'Je begint je shift met',kassaOp,'euro op je kassa ')
...
while einde == 'n':
   #Inlezen van variabelen en constanten
    ...
    #Inlezen van waardes
    print('Uw totaal te betalen  =', bedragP,'$')
    ontvangen=int(input('Geef het betaald bedrag:  '))
    print(20*'*')
    bedrag = ontvangen-tebetalen
    kassaBegin = kassaBegin + bedragP
    kasticket = str(input('Kassa ticket afprinten?: '))
    ...
    if kasticket == 'j':
        ...
        #Hier zit er een fout denk ik Wouter heb die niet kunnen vinden SORRY
        if (bedrag < 0):
            teweinigbetaald = tebetalen - ontvangen
            print('Uw heeft', teweinigbetaald ,' euro te weinig betaald: ')
            ontvangen=int(input('Geef aub betaald bedrag terug in:  '))
        rest = ontvangen-tebetalen
        print(20*'*')
        ...
        print(20*'=')
    einde = str(input('Programma afsluiten?: '))
    print(20*'*')
    
else:
    kassaBegin = kassaBegin + kassaOp
    print('Bedankt',naam)
    print('Bij begin was er',kassaOp,'in kassa.')
    print('Nu',kassaBegin,'in je kassa')
